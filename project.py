        ...
        for file in files_with_price:
            ...
            try:
                ...
                if all([product_col, price_col, weight_col]):
                    ...
                    data.extend(csv_data[[product_col, price_col, weight_col, 'price_per_kg', 'file']].values.tolist())
...
def main():
    loader = PriceLoader()
    data = loader.load_prices(os.getcwd())

    searcher = PriceSearcher(data)
    exporter = HtmlExporter(data)
    ...
    while True:
        ...
        if fragment.lower() == 'exit':
            ...
            break
        results = searcher.find_text(fragment)
        if results:
